...
void UAWingedCharacterMovement::PhysFlying(float deltaTime, int32 Iterations) 
{
	...
	while ((remainingTime >= MIN_TICK_TIME) && (Iterations < MaxSimulationIterations))
	{
		...
		AWingedAnimal* WingedAnimal = Cast<AWingedAnimal>(CharacterOwner);
		if (!WingedAnimal)
		{
			return;
		}
		...
		if (!HasAnimRootMotion() && !CurrentRootMotion.HasOverrideVelocity())
		{
			...
			WingedAnimal->FlyingTick(timeTick);
		}
